...
namespace ConcaveHull {
    public static class Hull {
        ...
        public static List<Line> GetConcave(UnityEngine.Vector3[] points, double concavity, int scaleFactor) {

            List<Node> nodes = new List<Node>();
            List<Node> unused_nodes = new List<Node>();
            List<Line> hull_edges = new List<Line>();
            List<Line> hull_concave_edges = new List<Line>();

            for (int i = 0; i < points.Length; i++) {
                nodes.Add(new Node(points[i].x, points[i].z, i));
            }
